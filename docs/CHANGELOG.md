...
## Phase E.4.8 — 2026-09-12: إصلاحات مراجعة عن بعد (IEP 0% · Avance · HS TAUX · Panier/Transport TAUX · حضورٌ مستحيل)

مراجعة كودٍ عن بعد لِـ`master..review/e4` رصدت خمس مسائل صحّة — لا تعديل
غير مرتبط، لا لمس `feature/shell-prototype` أو `ui2/paie/bulletin.py`
(شاشةٌ منفصلة، يُؤجَّل قرار التوحيد لِما بعد قبول E.4).

### §1/§2 — IEP: 0% نتيجةٌ صالحة + دِقّة صارمة للإدخال

`suggest_iep_taux` يُرجع صراحةً 0% تحت الحدّ الأدنى — التحقّق كان يطلب
`taux > 0` فيرفض هذه الحالة الصالحة خطأً. الإصلاح لا يكتفي بـ`>= 0`
(الخطر: نصٌّ فاسد كان يُقرأ صفراً صامتاً عبر `_dec` المتساهلة، فيصير
"صفراً شرعياً" زائفاً) — بل يميّز صراحةً بين ثلاث حالات عبر
`_Row.iep_taux_validity()`: `"blank"` (فراغٌ حقيقيّ، ناقص)، `"invalid"`
(نصٌّ غير رقميّ، ناقص أيضاً لكن **لا يُقرأ صفراً**)، `"ok"` (رقمٌ صريح،
قد يكون صفراً أو سالباً — السلبيّة وحدها تُرفَض في `_row_status`).

خليّة TAUX-IEP تصير `_RateEdit` مزوَّدةً بفاليديتور تفاعليّ
(‏`QRegularExpressionValidator`) يمنع الحروف من الدخول أصلاً؛ `_dec`
الجديدة `PZ.is_valid_rate_text` (نصٌّ رقميّ صريح: علامة سالبة + أرقام +
فاصل عشريّ اختياريّ) تحرس الطرفين معاً — `value()` تُرجع `""` (لا "0")
لنصٍّ فاسد، و`set_value()` تعرض النصّ الفاسد المحفوظ حرفياً بدل تصحيحه
صامتاً إلى "0,00". `_on_row_edit` صار يعتمد «الخليّة فارغة أم لا» لا
`val()` لتحديد `_iep_manual` — نصٌّ فاسد يبقى ظاهراً (يدويّاً)، لا
يُستبدَل صامتاً باقتراحٍ جديد يخفي الفساد.

### §3 — AVANCE: نوعٌ دلاليٌّ مخصَّص لا `libre` عامّ

`_avance_entry` كانت تُرسِل `{"type": "libre", ...}` — يخالف عقد E.4
الدلاليّ رغم وجود `lignes.LINE_TYPES["avance"]` المخصَّص. الإصلاح:
`{"type": "avance", "values": {"montant": ...}}`. تحقّقٌ مباشر بالمحرّك
يُثبت تطابق NET حرفياً قبل/بعد (نفس `Zone Z4`/`RETENUE`، بلا CNAS/IRG).
فصل النوع يزيل بنيوياً تصادم توقيع `("libre", Z4, RETENUE)` المحتمل مع
اقتطاعٍ حرٍّ عامّ في نفس المنطقة (كان الاعتماد على تطابق ترتيب الإرسال
فقط، لا فصلاً بنيوياً).

### §4 — HS TAUX: المعدّل الدقيق من المحرّك، لا مبلغ السطر ÷ الساعات

`row._amount ÷ heures` كان يُخفي فرقاً حقيقياً لساعاتٍ كسريّة صغيرة
(0.10h/0.33h مُثبَتتان بالاختبار) بسبب تقريب المبلغ لخانتين قبل القسمة.
`calc.SequenceResult` صار يحمل `heures_supp_taux` (معدّلٌ دقيقٌ موازٍ
`heures_supp_lignes`، بلا تقريب)؛ `lignes.LineView` صار يحمل حقلاً
اختيارياً `taux` (hs_50/hs_100 فقط). `_assign_computed_amounts` يُسنِد
`row._taux_exact` بنفس آلية `row._amount` (توقيع E.3-Review-2)، و
`_row_display_extra` يستهلكه مباشرةً — بلا معاملٍ ثابتٍ (1.5/2.0) يُعاد
اختراعه في Qt.

### §5 — Panier/Transport TAUX: يطابق سياسة التنسيب الفعليّة

‏`res.heures_presence` لا يطابق العامل الفعليّ المُستهلَك في GAIN تحت
AUCUN/PRORATA_JOURS (يبقى حساباً ساعياً موازياً بينما `facteur` الفعليّ
مبنيٌّ على الأيام أو ثابتٌ 1) — لم يظهر عملياً على الشاشة الحيّة (لا
تدعم اليوم سوى PRORATA_HEURES/MIXTE)، لكنه خطرٌ حقيقيّ على عملٍ قديمٍ
محفوظٍ بسياسةٍ أخرى يُعاد فتحه. `calc.SequenceResult` صار يحمل
`panier_transport_heures_equiv` (ساعاتٌ معادِلة للعامل الفعليّ أياً
كانت السياسة، محسوبةٌ من نفس `facteur` المُستعمَل للمبلغ) — مصدر
الحقيقة الوحيد لِـTAUX المعروض الآن، بلا تغيير أيّ صيغة ماليّة.

### §6 — حضورٌ مستحيل يمنع الإصدار النهائي فقط

تحقّقٌ إضافيّ جديد في `validation.py` (`_attendance_problems`): يقارن
غياب الأيام/الساعات وساعات التأخّر بـ`jours_mois`/`heures_mois`
الفعليّين للفترة (من `params_paie`)، فرداً فرداً وبالحدّ المُجمَّع (كسر
الغياب معاً ≤ 1، ومُنقِصات وقت الأجر مجتمعةً ≤ 1). **لا** تعديل أيّ
صيغة حساب في `calc.py`/`lignes.py`، **لا** حدّاً قانونياً جديداً
للساعات الإضافيّة. الحفظ (`_on_save`) لا يستشير هذا التحقّق إطلاقاً —
يبقى مسموحاً دائماً بعملٍ ناقص/مستحيل الحضور (§4)؛ الإصدار النهائي وحده
يُحجَب.

### الاختبارات

`ui2/hr/paie/tests` **302 OK** (277 + 25 اختباراً جديداً يغطّي الخمس
مسائل أعلاه صراحةً) — بلا تراجعٍ في أيّ اختبارٍ قائم. `programme/payroll/tests`
65 OK (بما فيها `test_prorata_mixte` كاملةً — لم يتأثّر أيّ سيناريو
منسوب موجود) · `programme/tests` 6 OK · `ui2/tests` 42 OK ·
`ui2/paie/tests` 17 OK (شاشةٌ أخرى، غير مُلامَسة) ·
`demos/ui2_paie_gallery.py --selftest` ALLOK · `git diff --check` نظيف.

### الملفات المتأثرة

`programme/payroll/calc.py` · `programme/payroll/lignes.py` ·
`ui/hr/paie/presentation.py` · `ui2/hr/paie/bulletin_template.py` ·
`ui2/hr/paie/validation.py` · `ui2/hr/paie/tests/test_bulletin_template.py`
· `docs/CHANGELOG.md`.
